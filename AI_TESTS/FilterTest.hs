...
import qualified SOLTest.Filter as F
import SOLTest.Filter (matchesCriterion)
import Generators ()
...
prop_emptyFilterSelectsAll tests =
  let spec = FilterSpec [] [] False
      (selected, excl) = F.filterTests spec tests
...
prop_filterIdempotent spec tests =
  let (selected1, _) = F.filterTests spec tests
      (selected2, _) = F.filterTests spec selected1
...
prop_filterPartitions spec tests =
  let (selected, filteredOut) = F.filterTests spec tests
...
prop_excludeOverridesInclude test =
  let cat = tcdCategory test
      ...
      (selected, _) = F.filterTests spec [test]
