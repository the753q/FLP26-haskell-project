...
executeCombined parserPath interpPath test = do
  ...
  if tcrResult parserReport /= Passed
    then return parserReport
      ...
      -- Early return if exit code wasn't 0
...
checkExecutable path = do
  ...
  case result of
    ...
    Right False -> return (Just (UnexecutedReason CannotExecute (Just ("File -" ++ path ++ "- does not exist!"))))
    Right True -> do
      perms <- getPermissions path
      if executable perms
        then return Nothing
        else return (Just (UnexecutedReason CannotExecute (Just ("File -" ++ path ++ "- is not executable!"))))
